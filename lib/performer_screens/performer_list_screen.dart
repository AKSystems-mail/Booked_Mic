...
// Create an instance here. Assumes initialization happens in main.dart
// If you manage state differently (Provider, Riverpod), inject/access it accordingly.
...
class _PerformerListScreenState extends State<PerformerListScreen> {
  ...
  void _updateAndNotifyPositions(List<QueryDocumentSnapshot> docs) {
     ...
     _lastNotifiedPositionNotifier.value.forEach((listId, position) {
        if (currentPositions.containsKey(listId) && currentPositions[listId] != null) {
           nextNotifierValue[listId] = currentPositions[listId];
        }
     });
     currentPositions.forEach((listId, position) {
        if (position != null && !nextNotifierValue.containsKey(listId)) {
           nextNotifierValue[listId] = position;
        }
     });

     if (!MapEquality().equals(_lastNotifiedPositionNotifier.value, nextNotifierValue)) {
        print("Updating tracked positions: $nextNotifierValue");
        _lastNotifiedPositionNotifier.value = nextNotifierValue;
     } else {
        print("No change in overall tracked positions.");
     }
  }
  ...
  Widget build(BuildContext context) {
    final Color appBarColor = Colors.blue.shade400;
    final Color fabColor = appBarColor; // Use variable
    if (currentUserId == null) {
       WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => RegistrationScreen()), (Route<dynamic> route) => false);
       });
    ...
    return Scaffold(
      ...
      floatingActionButton: FadeInUp(
         ...
         child: FloatingActionButton(
           ...
           backgroundColor: fabColor, // Use variable
