  ...
  String? _extractStateAbbr(Prediction prediction) {
    ...
    if (prediction.structuredFormatting?.secondaryText != null) {
      ...
      if (parts.length >= 2) {
        ...
        if (trimmedStatePart.length == 2 &&
            RegExp(r'^[a-zA-Z]+$').hasMatch(trimmedStatePart)) {
          return trimmedStatePart.toUpperCase();
        }
      }
    }
    print(
        "Warning: Could not reliably extract state from prediction: ${prediction.description}");
    return null;
  }
  ...
  Future<void> _createList() async {
    ...
    if (_selectedDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Please select a date for the list.')));
      return;
    }
    ...
    if (user == null) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('Error: No user logged in.')));
      return;
    }
...
// Simple format for now
  ...
  Widget build(BuildContext context) {
    ...
    if (googleApiKey == 'MISSING_API_KEY') {
      print(
          "FATAL ERROR: GOOGLE_PLACES_API_KEY key not found or empty in .env file! Using default 'MISSING_API_KEY'.");
      bodyContent = Center(
        child: Padding(
              ...
              child: GooglePlaceAutoCompleteTextField(
                ...
                getPlaceDetailWithLatLng: (Prediction prediction) {
                  ...
                  if (_selectedStateAbbr == null && mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        content:
                            Text('Could not automatically determine state.'),
                        backgroundColor: Colors.orange));
                  }
                },
